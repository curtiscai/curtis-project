...
/**
 * @author curtis
 * @desc 使用EasyExcel自定义数据格式
 * @date 2020-06-27
 * @email 397773935@qq.com
 * @reference
 */
public class CustomDataWriteTest {

    private static final Logger LOGGER = LoggerFactory.getLogger(CustomDataWriteTest.class);

    /**
     * EasyExcel的com.alibaba.excel.annotation.format包提供两个注解用于自定义数据格式：
     * 分别是日期和数值DateTimeFormat、NumberFormat
     ...
     */
    @Test
    public void testCustomDataWrite() {
        ...
        List<CustomData> datas = this.getConvertDatas(1_0000);
        /**
        ...
        try {
            excelWriter = EasyExcel.write(fileName, CustomData.class).build();
...
//        EasyExcel.write(fileName, DemoData.class).sheet("用户信息").doWrite(datas);
    ...
    private List<CustomData> getConvertDatas(int count) {
        List<CustomData> datas = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            CustomData customData = new CustomData();
            datas.add(customData);
            customData.setName("curtis" + (i + 1));
            customData.setAge(20 + (i % 10 + 1));
            customData.setHeight(BigDecimal.valueOf(180 + (i % 10 + 1)));
            String birthStr = "1990-01-" + ((i % 30) < 9 ? ("0" + (i % 30 + 1)) : (i % 30 + 1)) + "T00:00:00Z";
            customData.setBirth(Date.from(Instant.parse(birthStr)));
            customData.setSex(i % 2 == 0 ? "男" : "女");
        }
        return datas;
    }
}
